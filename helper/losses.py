...
class CategoryConLoss(nn.Module):

    def __init__(self, opt, temperature=0.07, contrast_mode='all',
                 base_temperature=0.07):
        super(CategoryConLoss, self).__init__()
        self.temperature = temperature
        self.contrast_mode = contrast_mode
        self.base_temperature = base_temperature
        #self.embeds = Embed(opt.t_dim, opt.t_dim//2)
        self.embed_s = Embed(opt.s_dim, opt.s_dim)
        self.embed_t = Embed(opt.s_dim, opt.t_dim)
        #self.embed_s = Head(opt.s_dim, opt.feat_dim) 

    def forward(self, feat_s, feat_t, fcl_s, fcl_t,labels=None, mask=None):
        ...
        embed_s1=self.embed_s(feat_s)
        fu_s=torch.matmul(embed_s1,fcl_s)
        
        #print('fu_s: ',fu_s.shape)
        #print(fcl_t.shape)
        embed_s2=self.embed_t(feat_s)
        fu_t=torch.matmul(embed_s2,fcl_t)
        
        #print('fu_t: ',fu_t.shape)
        
        
        anchor_feature = torch.cat([fu_s, fu_t], dim=0)
        anchor_count = 2
 
        #print('labels: ',labels.shape) # 256 1
        #print('anchor: ',anchor_feature.shape)    # 512 128   
...
#focal_loss func, L = -Î±(1-yi)**Î³ *ce_loss(xi,yi)

class focal_loss(nn.Module):
    def __init__(self,gamma=2, num_classes = 100, size_average=True):
    ...
    def forward(self, preds, labels):
        ...
        #focal_loss func, Loss = -¦Á(1-yi)**¦Ã *ce_loss(xi,yi)
        preds_softmax = preds_softmax.gather(1,labels.view(-1,1)) 
        preds_logsoft = preds_logsoft.gather(1,labels.view(-1,1))
        #print("good")
        #print(labels.shape) # [64]
        #print(self.alpha.shape) # [100, 1]
        #print(labels.view(-1).shape) #[64] 
        
        #print(labels.view(-1,1).shape) # [64, 1]
        #self.alpha = self.alpha.gather(0,labels.view(-1,1))
        #print(self.alpha.shape)
        # torch.pow((1-preds_softmax), self.gamma) Îªfocal lossÖÐ (1-pt)**¦Ã
