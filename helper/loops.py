...
import math
def train_vanilla(epoch, train_loader, model, criterion, optimizer, opt,prew=None):
    """vanilla training"""
    model.train()
...
def train_vanilla_tongji(epoch, train_loader, model, criterion, optimizer, opt):
    """vanilla training"""
    model.train()
    ...
    for idx, (input, target) in enumerate(train_loader):
        ...
        # ===================forward=====================
        output = model(input)
        loss,x = criterion(output, target,'train')
        ...
        pt.update(x.sum().item(),input.size(0))
        print('%.3f' % pt.val)
        #print('pt.sum: ',pt.sum)
        #print('pt.count: ',pt.count)
        # ===================backward=====================
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
...
def validate_prew(val_loader, model, criterion, opt,epoch):
    """validation"""
    batch_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
