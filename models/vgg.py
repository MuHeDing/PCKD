...
class VGG(nn.Module):
    ...
    def forward(self, x, is_feat=False, preact=False):
        # print(x.shape) #torch.Size([256, 3, 32, 32])
        h = x.shape[2]
        ...
        x = F.relu(x)
        f1 = x
        # print(f1.shape) # torch.Size([256, 128, 16, 16])
        x = self.pool1(x)
        x = self.block2(x)
        f2_pre = x
        x = F.relu(x)
        f2 = x
        # print(f2.shape)   # torch.Size([256, 256, 8, 8])
        x = self.pool2(x)
        x = self.block3(x)
        f3_pre = x
        x = F.relu(x)
        f3 = x
        # print(f3.shape)  #  torch.Size([256, 512, 4, 4])
        if h == 64:
            x = self.pool3(x)
        x = self.block4(x)
        f4_pre = x
        x = F.relu(x)
        f4 = x
        # print(f4.shape) # torch.Size([256, 512, 4, 4])
        x = self.pool4(x)
        # print(x.shape)  # torch.Size([256, 512, 1, 1])
        x = x.view(x.size(0), -1)
        f5 = x
        # print(x.shape)  # torch.Size([256, 512])
        x = self.classifier(x)    #  # torch.Size([256, 100])

        if is_feat:
            if preact:
                return [f0, f1_pre, f2_pre, f3_pre, f4_pre, f5], x
            else:
                return [f0, f1, f2, f3, f4, f5], x,self.classifier
        else:
            return x
